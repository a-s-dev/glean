...
impl SettingsClient for Client {
    ...
    // TODO: Adjucst this to work with the real nimbus schema
    fn get_experiments(&self) -> Result<Vec<Experiment>> {
        let path = format!(
...
// A We could use mockito (not mockiato) to mock the network requests here
// and ensure that we handle all the edge cases
